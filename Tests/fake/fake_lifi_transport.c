...
static Fake_LiFi_Link_t* find_link_by_transmitter(
...
void Fake_LiFi_Transmit(Fake_LiFi_Link_t* link) {
  ...
  for (uint8_t i = transmitter->current_tx_byte_index; i < tx_total_bytes;
       i++) {
    link->receiver->rx_byte =
    ...
    if (link->receiver->on_byte_received != NULL &&
      ...
      link->receiver->on_byte_received(
          link->receiver->on_byte_received_callback_context);
    }
  }
  ...
  if (transmitter->on_buffer_transmitted != NULL &&
    ...
    transmitter->on_buffer_transmitted(
        transmitter->on_buffer_transmitted_callback_context);
  }
}
...
void Fake_LiFi_RunUntilIdle(void) {
  ...
  for (uint8_t i = 0; i < FAKE_LIFI_MAX_LINKS; i++) {
    if (registered_links[i].transmitter != NULL &&
        registered_links[i].transmitter->is_busy &&
...
void Fake_LiFi_Transmitter_TransmitBuffer_Callback(
  ...
  TEST_ASSERT_TRUE_MESSAGE(length <= LIFI_TX_BUFFER_SIZE,
                           "fake transmit buffer is too small");
  TEST_ASSERT_FALSE_MESSAGE(transmitter->is_busy,
                            "fake transmitter already has pending tx");

  memcpy(transmitter->tx_buffer, buffer, length);
  transmitter->tx_total_bytes = length;
  transmitter->current_tx_byte_index =
      1;  // skip preambule, matching physical receiver behavior
  transmitter->is_busy = true;
}

void LiFi_Receiver_ReadBuffer(LiFi_Receiver_t* receiver) { (void)receiver; }
