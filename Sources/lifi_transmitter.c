...
static void copy_to_tx_buffer(uint8_t* tx_buffer, const uint8_t* buffer,
                              uint8_t length) {
...
void LiFi_Transmitter_ToConfirmationMode(LiFi_Transmitter_t* transmitter) {
  __HAL_TIM_SET_PRESCALER(transmitter->htim,
                          LIFI_TRANSMIT_CONFIRM_TIMER_PRESCALER);
  transmitter->htim->Instance->EGR = TIM_EGR_UG;
}
...
void LiFi_Transmitter_TimerCallback(LiFi_Transmitter_t* transmitter) {
  ...
  if (transmitter->current_half_bit_index >= 0 &&
      transmitter->current_half_bit_index < 16) {
    uint8_t state =
        transmitter->manchester_buffer[transmitter->current_half_bit_index];
    HAL_GPIO_WritePin(transmitter->gpio_port, transmitter->gpio_pin,
